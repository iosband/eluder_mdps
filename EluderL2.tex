...
% \title{Posterior Sampling for Reinforcement Learning and the Eluder Dimension}
\title{Model-based Reinforcement Learning \\ and the Eluder Dimension}
\author{Ian Osband, Benjamin Van Roy \\ \\ Stanford University}
...
This represents the first unified framework for model-based reinforcement learning and provides state of the art guarantees in several important settings.
...
% \begin{itemize}
% 	\item Reinforcement learning problem on MDPs.
% 	\item Lots of settings have large or even infinite state spaces.
% 	\item Some people have done work showing you can do stuff in certain cases.
% 	\item Regret bounds are good, they imply PAC bounds and make sense for online implementations
% 	\item Some people have done regret bounds of various kinds
% 	\item Eluder dimension in bandits gives a general framework for MAB.
% 	\item We extend this to reinforcement learning in quite a general setting.
% \end{itemize}

We consider an agent that interacts with an uncertain environment while trying to maximize its cumulative rewards through time \cite{burnetas1997optimal}.
We model this environment as a Markov decision process (MDP) where the agent is unsure of the true reward and transition but does know that they lie within some functional families $\Rc$ and $\Pc$.
Efficient reinforcement learning seeks algorithms which balance exploration with exploitation, so that the agent's performance is guaranteed to be close to the unknown optimal control in some sense \cite{lai1985asymptotically,kaelbling1996reinforcement}.

The main focus in this literature has been to develop algorithms for the \emph{tabula rasa} setting, where little prior knowledge of the environment is assumed beyond the state and action spaces.
...
Our results provide a unified framework for the analysis of model-based reinforcement learning and provide new state of the art bounds in several important problem settings.
...
% We present the first unified analysis for efficient regret bounds in reinforcement learning across MDPs with general rewards in $\Rc \subseteq \Pc^{C_\Rc,\sigma_\Rc}_{\Sc \times \Ac, \Real}$ and transitions in $\Pc \subseteq \Pc^{C_\Pc,\sigma_\Pc}_{\Sc \times \Ac, \Sc}$. in terms of the dimensionality of $\Rc$ and $\Pc$.
% To account for the complexity of the reinforcement learning problem, we extend the definition of Eluder dimension from the bandit setting and obtain expected regret bounds $\tilde{O}\left( \sigma_\Rc \sqrt{d_K(\Rc)d_E(\Rc,T^{-1})T} + \sigma_\Pc \Exp[K^*] \sqrt{d_K(\Pc) d_E(\Pc,T^{-1}) T} \right)$.
% This general analysis provide new state of the art guarantees when specified to several problem settings, including linear quadratic systems.
% What is more, we show that the conceptually simple and computationally efficient algorithm PSRL attains these bounds irrespective of the particular structure of $\Rc$ and $\Pc$.

% We present regret bounds for \emph{posterior sampling for reinforcement learning} for an extremely general class of MDPs.
% The $\tilde{O}\left( \sigma \sqrt{d_E(\Rc)d_K(\Rc)T} + \sigma \Exp[K^*] \sqrt{d_E(\Pc) d_K(\Pc) T} \right)$ Bayesian regret bounds 
% provide new state of the art guarantees across several problem settings, including linear quadratic systems.
% We extend the notion of Eluder dimension from bandit problems to reinforcement learning to effectively express the complexity of the learning problem.
% PSRL is conceptually simple, computationally efficient and can easily incorporate prior knowledge.
% Unlike UCB methods, the \emph{algorithm} is separated from the \emph{analysis} so that efficient learning is attained even without careful construction of confidence sets or tight bounds on the Eluder dimension.
