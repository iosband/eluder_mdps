...
\end{eqnarray}
Where $\tilde{D}(\Fc)$ is shorthand for $ 1 + \tau C_\Fc d_\Fc + 8\sqrt{d_\Rc(4C_\Fc + \sqrt{2\sigma_\Fc^2 \log(32T^3)})} + 8\sqrt{2 \sigma_\Fc^2 n_\Fc d_\Fc T}$ for $\Fc$ equal to $\Rc$ or $\Pc$ respectively.
\end{theorem}
...
Under the assumptions of Theorem \ref{thm: main regret} and writing $\kappa_\Fc := {\rm dim}_K(\Fc)$:
\begin{equation}
	\Exp[{\rm Regret}(T,\pi^{PS},M^*)] = \tilde{O} \left( \ \sigma_\Rc \sqrt{\kappa_\Rc d_\Rc T}
		+ \Exp[K^*] \sigma_\Pc \sqrt{\kappa_\Pc d_\Pc T} \ \right)
\end{equation}
...
To clarify the assymptotic dependence we finally introduce another common notion of dimension.
\begin{mydef}
\label{def: kol}
